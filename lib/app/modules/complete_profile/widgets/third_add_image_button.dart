...
class ThirdAddImageButton extends StatelessWidget {
  ThirdAddImageButton({
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        if (completeProfileController.imgUrl3.value == '') {
          final file =
          ...
          try {
            completeProfileController.UpdateImgUrl3(croppedImg.path);
            await referenceImage.putFile(File(croppedImg.path),
                SettableMetadata(contentType: 'image/jpeg'));
            completeProfileController.UpdateImgDownloadUrl3(
                await referenceImage.getDownloadURL());
          } catch (error) {
            print(error);
          }
        } else {
          completeProfileController.UpdateImgUrl3('');
        }
      },
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            Obx(
              () => completeProfileController.imgUrl3.value == ''
                  ? Container(
                      height: 240.h,
                      width: 180.w,
                      margin: EdgeInsets.only(right: 10.w),
                  ...
                  : Container(
                      ...
                      child: Image.file(
                        File(completeProfileController.imgUrl3.value),
                        fit: BoxFit.cover,
                      ),
                    ),
            ),
